...
We will now reproduce the plot that shows the top ten cities in highest amounts raised in political contributions in California during the 2016 US Presidential election.


```{r challenge2, echo=TRUE, out.width="100%"}
knitr::include_graphics(here::here("images", "challenge2.png"), error = FALSE)
```

## Taking on Challenge 2

First, we read the two dataframes
...
Only the zip codes of California are required. Specifically, we want to know the primary cities corresponding to the zips
```{r}
zips_CA <- zips_df %>%
...
We chose the method that displays the same type of graph for multiple candidates. In this example, we filtered on just the 2 candidates (Hillary and Trump). If it would be desirable to analyse info of more than two candidates, the relevant ones can simply be added to the filter statement.
